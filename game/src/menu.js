var menuState = {
  create: function () {
    ...
    this.connectButton.scale.setTo(0.8, 0.8);
    ...
    this.creditsButton = game.add.button(
      400,
      360,
      "credits_button",
      this.credits,
    ...
    if (window.ethereum) {
      try {
        ...
        if (playeraddress) {
          ...
          this.registerbutton.scale.setTo(0.3, 0.3);
          this.tweenButtons(this.playButton);
        }
      } catch (error) {
  ...
  displayScores: function () {
    var lastScoreText = game.add.text(game.world.centerX, 30, "Last Run:", {
      font: "24px Arial",
      fill: "#a0a0a0",
    });
    lastScoreText.anchor.setTo(0.5, 0.5);

    var gapHeight = 60;
    var lastScoreValue = game.add.text(
      game.world.centerX,
      lastScoreText.y + gapHeight,
      game.global.score + " meters",
      {
        font: "32px Arial",
        fill: "#ffffff",
      }
    );
    lastScoreValue.anchor.setTo(0.5, 0.5);
  },
